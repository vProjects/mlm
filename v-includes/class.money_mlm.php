	...
	class money_mlm
	{
		...
		*/
		function receivingAmount($distributed_amount,$product_id,$quantity,$membership_id){
			...
			insertDebitAmount($membership_id,$product_id,$quantity,$date,$getting_amount,$new_system_balence,0,"");
		...
		*/
		...
		function amountForParent($membership_id,$product_id,$quantity,$distributed_amount,$rest_amount,$level){
			...
			if($parent_id[0]['parent_id'] != "")
			{
				...
				//inserting values in money transfer log table
				$result = $this->manage_data->
				insertDebitAmount($parent_membership_id[0]['membership_id'],$product_id,$quantity,$date,$amount_debited,$new_system_balence,$frozen_money,"");
         ...
         function insertPaypalPayment($orderId,$totalPrice,$memberid){
            ...
            foreach($arr as $key=>$value)
            {
                ...
                if(substr($key,0,10) == 'no_product')
                {
					...
					$result = $this->manage_data->insertPurchaseInfo($orderId,$product_id,$value,$date,$payment_method[0]['payment_method'],$payment_status);
                }
				...
				else if($value == 'mojolife')
				{
					...
					//inserting values in payment status
					$result = $this->manage_data->insertPurchaseInfo($orderId,$coupon_id,1,$date,$payment_method[0]['payment_method'],$payment_status);
				}
            }
			/*
